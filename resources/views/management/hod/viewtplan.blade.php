...
<body>
    @include('logout')
    @include('management.hod.sidebar')
...
<tbody>
    ...
    <form action="{{route('activate', $tplan->id)}}" method="POST">
        ...
        @endif
        @if($tplan->approved_by == "1")
        <button type="submit" class="btn btn-danger">Dispprove</button>
        @endif
    </form>
    </td>
    {{-- <td>
...
@if(auth()->user()->is_hod == '2')
@foreach($htplan as $tplan)
...
@if(auth()->user()->is_hod == '3')
@foreach($ltplan as $tplan)
